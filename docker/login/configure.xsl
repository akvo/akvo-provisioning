...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="//ds:subsystem/ds:datasources/ds:datasource[@jndi-name='java:jboss/datasources/KeycloakDS']">
    <datasource jndi-name="java:jboss/datasources/KeycloakDS" enabled="true" use-java-context="true" pool-name="KeycloakDS" use-ccm="true">
      <connection-url>jdbc:postgresql://${env.PSQL_PORT_5432_TCP_ADDR}:${env.PSQL_PORT_5432_TCP_PORT:5432}/${env.PSQL_DATABASE:keycloak}</connection-url>
      <driver>postgresql</driver>
      <security>
        <user-name>${env.PSQL_USERNAME:keycloak}</user-name>
        <password>${env.PSQL_PASSWORD:password}</password>
      </security>
      <validation>
