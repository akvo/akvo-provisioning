...
def create_hiera_facts(use_sudo=False):
    ...
    if env.environment != 'localdev':
        # skip this step for vagrant VMs
        config_dir = os.path.join(os.environ['AKVO_CONFIG_DIR'])
        put(config_dir, '/puppet/hiera/', use_sudo=use_sudo)

    run_method('for d in `find /puppet/checkout/config -type d -mindepth 1`; do  mkdir -p `basename $d`; done ')
    #run_method('for f in `find -type f /puppet/checkout/config/`; do  ln -s $f; done')
